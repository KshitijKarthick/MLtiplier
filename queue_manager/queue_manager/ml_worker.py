...
from config_manager.schema import Job, JobStatus, MLWorkerInput, JobStatusState, MLWorkerResult
from config_manager.redis import HOST
from config_manager.job import JOB_QUEUE_NAME


class MLWorkerQueueManager:
    def __init__(self, job_fn: Callable, host: AnyStr = HOST, queue_name: AnyStr = JOB_QUEUE_NAME,
                 polling_interval=1,
    ...
    def set_job_status(self, job_status):
        self.redis.setex(name=job_status.job_id, value=json.dumps(asdict(job_status)),
                         time=self.job_status_timeout)

    def run(self, indefinite_wait=True, max_connection_errors=5, connection_error_secs=5,
            redis_error_secs=1):
        redis_error_count = 0
        while True:
            try:
                ...
                try:
                    ...
                    job_status = JobStatus(
                        ...
                        result=ml_worker_result, input=job,
                    )
                    self.set_job_status(job_status=job_status)
                except (MLPredictionException, BaseException) as e:
                    if job.retries < self.max_retries:
                        self.logger.exception(f'Exception occurred: retry {job.retries} - {e!s}')
                        job.retries = job.retries + 1
                        self.insert_job_at_start(job=job)
                    else:
                        error_message = f'Max number of retry failures. Job: {job.job_id}'
                        self.logger.exception(error_message)
                        job_status = JobStatus(
                            job_id=job.job_id, status=JobStatusState.FAILURE,
                            input=job, error_message=error_message
                        )
                        self.set_job_status(job_status=job_status)
